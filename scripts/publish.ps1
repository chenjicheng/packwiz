...
#
# 流程：
#   1. 校验 pack.toml 和 server.json 存在
#   2. 从 dev 合并到 main
...
Write-Host "  Packwiz 发布" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not (Test-Path $PackToml)) {
    Write-Host "[错误] pack.toml 不存在" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "部署状态: https://github.com/chenjicheng/packwiz/actions" -ForegroundColor Gray
}
finally {
    Pop-Location
}
